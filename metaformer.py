...
class MetaFormerFPN(nn.Module):
    def __init__(self, num_classes=1000, pretrained='ImageNet', pretrained_weights=None):
        super().__init__()

        # Implement backbone architecture
        self.encoder = caformer_s18(num_classes=num_classes, pretrained=pretrained, pretrained_weights=pretrained_weights)
    ...
    def forward(self, x):
        ...
        x, features = self.encoder.forward_features(x)
...
def caformer_s18(num_classes=0, pretrained='ImageNet', pretrained_weights=urls["ImageNet1k"], **kwargs):
